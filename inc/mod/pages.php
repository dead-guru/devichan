...
use DeVichan\Data\Queries\{IpNoteQueries, UserPostQueries, ReportQueries};
use DeVichan\Functions\{Format, Net};
use DeVichan\Data\Driver\{CacheDriver, LogDriver};
...
function mod_edit_board(Context $ctx, $boardName) {
    ...
    if (isset($_POST['title'], $_POST['subtitle'])) {
        ...
        DeVichan\Functions\Theme\rebuild_themes('boards');
...
function mod_news(Context $ctx, $page_no = 1) {
    ...
    if (isset($_POST['subject'], $_POST['body'])) {
        ...
        DeVichan\Functions\Theme\rebuild_themes('news');
...
function mod_user_posts_by_ip(Context $ctx, string $cip, ?string $encoded_cursor = null) {
    ...
    if (hasPermission($config['mod']['view_ban'])) {
        $args['bans'] = Bans::find($ip, false, true, null, false);
    }
...
function mod_edit_ban(Context $ctx, $ban_id) {
    ...
    $args['bans'] = Bans::find(null, false, true, $ban_id, false);
    $args['ban_id'] = $ban_id;
    $args['boards'] = listBoards();
...
function mod_bans(Context $ctx) {
    ...
    if (isset($_POST['unban'])) {
        ...
        DeVichan\Functions\Theme\rebuild_themes('bans');
...
function mod_move_reply(Context $ctx, $originBoard, $postID) {
    ...
    if (isset($_POST['board'])) {
        ...
        // trigger themes
        DeVichan\Functions\Theme\rebuild_themes('post', $targetBoard);
        // mod log
...
function mod_ban_post(Context $ctx, $board, $delete, $post, $token = false) {
    ...
    if (isset($_POST['new_ban'], $_POST['reason'], $_POST['length'], $_POST['board'])) {
        ...
        if (isset($_POST['public_message'], $_POST['message'])) {
            ...
            buildIndex();
            // Rebuild themes
            DeVichan\Functions\Theme\rebuild_themes('post-delete', $board);
        }
...
function mod_edit_post(Context $ctx, $board, $edit_raw_html, $postID) {
    ...
    if (isset($_POST['name'], $_POST['email'], $_POST['subject'], $_POST['body'])) {
        ...
        DeVichan\Functions\Theme\rebuild_themes('post', $board);
...
function mod_deletebyip(Context $ctx, $boardName, $post, $global = false) {
    ...
    while ($post = $query->fetch(PDO::FETCH_ASSOC)) {
        ...
        DeVichan\Functions\Theme\rebuild_themes('post-delete', $board['uri']);
...
function mod_rebuild(Context $ctx) {
    ...
    if (isset($_POST['rebuild'])) {
        ...
        if (isset($_POST['rebuild_themes'])) {
            ...
            DeVichan\Functions\Theme\rebuild_themes('all');
        }
...
function mod_themes_list(Context $ctx) {
    ...
    while ($file = readdir($dir)) {
        if ($file[0] != '.' && is_dir($config['dir']['themes'] . '/' . $file)) {
            $themes[$file] = DeVichan\Functions\Theme\load_theme_config($file);
        }
    }
    closedir($dir);
...
function mod_theme_configure(Context $ctx, $theme_name) {
    ...
    if (!$theme = DeVichan\Functions\Theme\load_theme_config($theme_name)) {
    ...
    if (isset($_POST['install'])) {
        ...
        if (isset($theme['install_callback'])) {
            $ret = $theme['install_callback'](DeVichan\Functions\Theme\theme_settings($theme_name));
            if ($ret && !empty($ret)) {
    ...
    $settings = DeVichan\Functions\Theme\theme_settings($theme_name);
...
function mod_theme_rebuild(Context $ctx, $theme_name) {
    ...
    DeVichan\Functions\Theme\rebuild_theme($theme_name, 'all');
